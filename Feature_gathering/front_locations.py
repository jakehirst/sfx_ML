...
def get_average_node_locations(filepath):
    ...
    if(len(avg_locations) == 1): #if there is only 1 front,(its an edge crack) 
        ...
        # print('file to open = ' + folder + folder.split("\\")[-2] + ".odb_initiation_info.json")
        f = open(folder + '\\' +folder.split("\\")[-1] + ".odb_initiation_info.json",'r')
        data = json.load(f)
        f.close()
        initiation_cite = data[1][0]
        avg_locations["front 1"] = initiation_cite
...
def get_crack_len(outer_surface_nodes, main_side_nodes, node_locations, front_nodes):
    ...
    min_distance = np.inf
    while(len(main_side_surface_nodes.keys()) > 0):
        # if(np.array_equal(current_node,[121.125465, 18.1295223, 59.8404655])):
        #     print("here")
        for node in main_side_surface_nodes.keys():
            distance = get_euclidean_distance(main_side_surface_nodes[node], current_node)
            if(min_distance_and_node == None or distance < min_distance):
        ...
        result_key = next((key for key, value in main_side_surface_nodes.items() if np.array_equal(current_node,value)), None)        
        #print(f'min distance = {min_distance} for node {current_node}')
        # if(min_distance > 20):
        #     print("here")
        # if(np.array_equal(current_node, [129.114319, 42.5948753, 95.9394073])):
        #     print('here')
        # if(len(main_side_surface_nodes) < 10):
        #     print('here')
        ordered_main_side_surface_nodes.append(min_distance_and_node)
        current_node = main_side_surface_nodes[min_distance_and_node[1]]
        main_side_surface_nodes.pop(min_distance_and_node[1])
        min_distance = np.inf
    ...
    ''' get rid of any outliers where the current_node jumps from one end of the crack to the other '''
    filtered_list = [arr for arr in ordered_main_side_surface_nodes if arr[0] <= 3.0]
    total_len = sum(sublist[0] for sublist in filtered_list)
    # print('\nfiltered list: \n')
    # print(filtered_list)
    print(f'\n ########################################################### {len(ordered_main_side_surface_nodes) - len(filtered_list)} rows taken out! #############################################################\n')
    return total_len
...
def get_final_front_locations(simulation_folder, simulation):
    ...
    front_0 = np.empty(0)
    front_1 = np.empty(0)
    #print('front locations = ' + str(front_locations))
...
def get_main_side_outer_and_inner_surface_nodes(folder_path, simulation, max_step_uci):
    ...
    inner = False
    locations = False
    # simulation = folder_path.split('\\')[-2]

    # f = open(folder_path + simulation + f"\\Step{max_step_uci[0]}_UCI_{max_step_uci[1]}_Dynamic.inp",'r')
    # f = open(folder_path + f"Step{max_step_uci[0]}_UCI_{max_step_uci[1]}_Dynamic.inp",'r')
    if(folder_path.endswith('\\')):
        f = open(folder_path + f"{simulation}\\{simulation}_Stp{max_step_uci[0]}_UCI_{max_step_uci[1]}_full.inp",'r')
    else:
        f = open(folder_path + f"\\{simulation}_Stp{max_step_uci[0]}_UCI_{max_step_uci[1]}_full.inp",'r')
    ...
    for line in f.readlines(): 
        ...
        #     main = True
        if(line.startswith("*Nset,Nset=ALL_MAIN_SIDE_NODES")):
            main = True
        ...
        #     outer = True
        if(line.startswith("*Nset,Nset=OUTER_SURF")):
            outer = True
        ...
        #     inner = True
        if(line.startswith("*Nset,Nset=INNER_SURF")):
            inner = True
