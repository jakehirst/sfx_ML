...
def get_crack_locations(folder_path, key, max_step, max_uci):
    ...
    filepath = folder_path + key + "\\Step" + str(max_step) + "_UCI_" + str(max_uci) + "_Dynamic.inp"
    ...
    with open(filepath, 'r') as file:
        for row in file:
            ...
            if(row.startswith("*Nset, nset=ALL_MAIN_SIDE_NODES, instance=PART-1-1")):
                main = True
                mate = False
                continue
            # elif(row.startswith("*Nset,Nset=mate_side_a")):
            elif(row.startswith("*Nset, nset=ALL_MATE_SIDE_NODES, instance=PART-1-1")):
                mate = True
                main = False
                continue
            elif(row.startswith("*Nset, nset=ALL_NODES, instance=PART-1-1")):
                main = False
                mate = False
                break
