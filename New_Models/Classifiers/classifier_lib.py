...
def bin_labels(unbinned_labels, label, num_bins):
    all_labels = {}
    ...
    clipped_unbinned_labels = np.clip(unbinned_labels.flatten(), min_value, max_value)
    # binned_labels = pd.cut(unbinned_labels[label], bins=np.linspace(min_value, max_value, num_bins + 1), labels=False, include_lowest=True)
    binned_labels = pd.cut(clipped_unbinned_labels, bins=np.linspace(min_value, max_value, num_bins + 1), labels=False, include_lowest=True)
